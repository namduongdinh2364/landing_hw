        ...
        while not rospy.is_shutdown():
            if self.cv_image is not None:
                ...
                if np.all(ids is not None):
                    ...
                    for id in dir_marker_ids:
                        if id in ids:
                            list_size_markers.append(aruco_marker[str(id)]['size'])
                    list_size_markers.sort(reverse = True)

                    """ Get ID base on the size. """
                    for marker_id in aruco_marker:
                        if aruco_marker[marker_id]['size'] == list_size_markers[0]:
                            ids_target = aruco_marker[marker_id]['id']
                    ...
                    if ids_target is not None:
                        for i in range(0, ids.size):
                            if ids[i] == ids_target:
                                ...
                                markerLength = list_size_markers[0]
